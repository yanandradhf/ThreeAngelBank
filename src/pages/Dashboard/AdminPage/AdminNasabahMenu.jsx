  ...
  const fetchData = async () => {
    try {
      ...
      const nasabahWithAccountCount = nasabahRes.data.map((nasabah) => {
        const accountCount = rekeningRes.data.filter(
          (rekening) =>
      ...
      transaksiRes.data.forEach((transaction) => {
        ...
        if (accountNumber) {
          transactionCounts[accountNumber] =
            (transactionCounts[accountNumber] || 0) + 1;
        }
      });
      ...
      const accountsWithTransactions = Object.entries(transactionCounts)
        .map(([accountNumber, count]) => {
          const account = rekeningRes.data.find(
      ...
      <div className="mb-4">
        ...
        {searchTerm && (
          <p className="text-xs text-gray-500 mt-1">
            Ditemukan {filteredNasabah.length} nasabah dari pencarian "
            {searchTerm}"
          </p>
        )}
      </div>
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center text-xs text-gray-600">
                        ...
                        <span className="truncate">
                          {nasabah.email || `user${index + 1}@email.com`}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center text-xs text-gray-600">
                        ...
                        <span>
                          {nasabah.birthDate
                            ? `${formatDate(nasabah.birthDate)} (${calculateAge(
                                nasabah.birthDate
                              )} thn)`
                            : "01/01/1990 (34 thn)"}
                        </span>
...
function RekeningListCard({ rekeningList, rekeningCount, totalDana }) {
  ...
  const filteredRekening = allRekening.filter((rekening) => {
    ...
    const matchesSearch =
      ...
      (rekening.owner_name || rekening.account_holder || "")
        .toLowerCase()
        .includes(searchLower) ||
    ...
    const matchesStatus =
      filterStatus === "all" ||
      (rekening.account_status || "").toLowerCase() ===
  ...
  const currentTotalDana =
    totalDana > 0
      ? totalDana
      : dummyRekening.reduce((sum, rek) => sum + rek.account_balance, 0);
      ...
      <div className="mb-6 space-y-4">
        ...
        {(searchTerm || filterStatus !== "all" || filterType !== "all") && (
          <p className="text-xs text-gray-500">
            Ditemukan {filteredRekening.length} rekening dari{" "}
            {allRekening.length} total rekening
          </p>
        )}
      </div>
      ...
      <div className="space-y-4 max-h-96 overflow-y-auto mb-6">
        {currentRekening.length > 0 ? (
          currentRekening.map((rekening, index) => (
            <div
              key={rekening.id || rekening.account_number || index}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <h3 className="text-lg font-bold text-gray-800 tracking-wider">
                      {rekening.account_number ||
                        `ACC${String(index + 1).padStart(6, "0")}`}
                    </h3>
                  </div>
                  <div className="space-y-1">
                ...
                <div className="flex flex-col justify-between space-y-3">
                  <div className="space-y-1">
                    ...
                    <span
                      className={`inline-block px-3 py-2 text-sm font-semibold rounded-lg ${
                        (rekening.account_status || "Active").toLowerCase() ===
                        "active"
            ...
            {searchTerm || filterStatus !== "all" || filterType !== "all" ? (
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Data rekening akan muncul di sini setelah ditambahkan
                  </p>
                </div>
              </div>
            )}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between pt-4 border-t border-gray-100">
          <div className="text-sm text-gray-500">
            ...
            {currentRekening.length} dari {filteredRekening.length} rekening
          </div>
